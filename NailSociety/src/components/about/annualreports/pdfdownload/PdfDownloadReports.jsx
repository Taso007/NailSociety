...
import './pdfdownloadreports.css';
import { getStorage, ref, getDownloadURL } from "firebase/storage";
import { useTranslation } from 'react-i18next';

function PdfDownloadReports({ title, file}) {  
  ...
  return (
    <div className='pdf-container_reports'>
      <div className='pdf-head-container_reports'>
        <h3 className='old-standard-tt-regular text-wrap text-break'>{title}</h3>
      </div>
        <div className="pdf-download_reports mt-3" onClick={downloadFile}>
          <div className="pdf-download__icon_reports">
          ...
          <div className="pdf-download__text_reports old-standard-tt-regular">
            {t('download')}
          </div>
        </div>
    </div>
...
export default PdfDownloadReports;
